...
class RRT_Connect_Star():
    ...
    def expand_path(self):
        for i in range(self.iter_max):
            ...
            self.rewrite(self.vertex1, node_new1)
            if not self.utils.is_collision(node_new1.parent, node_new1):
                self.vertex1.append(node_new1)

                node_sp2 = node_new1
                node_near2 = self.get_nearest(self.vertex2, node_sp2)
                node_new2 = self.steer(node_near2, node_sp2)
                self.rewrite(self.vertex2, node_new2)
                if not self.utils.is_collision(node_new2.parent, node_new2):
    ...
    def rewrite(self, vertex, node_new):
        near_list = []

        for node in vertex:
            if self.get_dist_theta(node, node_new)[0] <= self.search_radius:
                near_list.append(node)
        
        for node in near_list:
            new_cost = self.CostDict[node] + self.get_dist_theta(node, node_new)[0]
            if new_cost < self.CostDict[node_new]:
                node_new.parent = node
                self.CostDict[node_new] = new_cost

        for node in near_list:
            new_cost = self.CostDict[node_new] + self.get_dist_theta(node_new, node)[0]
            if new_cost < self.CostDict[node]:
                node.parent = node_new
                self.CostDict[node] = new_cost
...
def main():
    ...
    rrt_connect = RRT_Connect_Star(x_start, x_goal, 10000, 0.95, 0.5, 1)
