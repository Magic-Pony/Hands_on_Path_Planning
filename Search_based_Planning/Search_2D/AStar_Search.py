...
class Astar():
    ...
    def search_path(self):
        ...
        heapq.heappush(self.OpenList, (0 + self.get_h_cost(self.start_node), self.start_node))
                ...
                if node_cur == self.end_node:
                    ...
                    for node_nb in self.get_neighbor(node_cur):
                        if node_nb not in self.CloseList:
                            nb_cost_g = self.CostDict[node_cur] + self.get_cost(node_cur, node_nb)
                            nb_cost_h = self.get_h_cost(node_nb)
                            nb_cost = nb_cost_g + nb_cost_h
                            if node_nb in self.CostDict:
                                if nb_cost < self.CostDict[node_nb] + nb_cost_h:
                                    self.ParentDict[node_nb] = node_cur
                                    self.CostDict[node_nb] = nb_cost_g
    ...
    def get_h_cost(self, node_nb):
        x_diff = abs(node_nb[0] - self.end_node[0])
        y_diff = abs(node_nb[1] - self.end_node[1])
        return math.sqrt(2) * min(x_diff, y_diff) + max(x_diff, y_diff) - math.sqrt(2)* max(x_diff, y_diff)
        # return math.sqrt((node_nb[0] - self.end_node[0])**2 + (node_nb[1] - self.end_node[1])**2)
...
def main():
    ...
    if astar.PathList[0] == astar.end_node:
        print("Path found!")
        print(f"Path length: ", len(astar.PathList))
        print(f"Visited nodes: ", len(astar.VisitList))
        print(f"Cost:", astar.CostDict[astar.PathList[0]])
    else:
        print("Path not found!")
    
    plot = plotting.Plotting(s_start, s_goal)
    plot.animation(astar.PathList, astar.VisitList, "Astar Searching")  # animation
