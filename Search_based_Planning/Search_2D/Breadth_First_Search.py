...
class BFS():
    ...
    def search_path(self):
        ...
        while self.OpenList:
            node_cur = self.OpenList.popleft()
...
def main():
    ...
    bfs = BFS(s_start, s_goal, 'Euclidean')
    bfs.search_path()
    bfs.get_path()
    bfs.get_visit()

    if bfs.PathList[0] == bfs.end_node:
        print("Path found!")
        print(f"Path length: ", len(bfs.PathList))
        print(f"Visited nodes: ", len(bfs.VistList))
        print(f"Cost:", bfs.CostDict[bfs.PathList[0]])
    else:
        print("Path not found!")
    
    plot = plotting.Plotting(s_start, s_goal)
    plot.animation(bfs.PathList, bfs.VistList, "Breadth-first Searching (BFS)")  # animation
